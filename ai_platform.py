"""Small provider-neutral chat client for OpenAI-compatible, Anthropic, and Gemini APIs."""
...
class AIConfig:
    ...
    def from_env(cls) -> AIConfig:
        ...
        return cls(provider, api_key, os.getenv("AI_MODEL", default_model).strip() or default_model, os.getenv("AI_BASE_URL", default_base).rstrip("/"), timeout)
...
class HTTPAIClient:
    ...
    def generate(self, system: str, user: str) -> str:
        ...
        if self.config.provider == "openai":
            data = self._post(f"{self.config.base_url}/chat/completions", {"model": self.config.model, "temperature": 0, "messages": [{"role": "system", "content": system}, {"role": "user", "content": user}]}, {"Authorization": f"Bearer {self.config.api_key}"})
            choices = data.get("choices")
            message = choices[0].get("message") if isinstance(choices, list) and choices and isinstance(choices[0], dict) else None
            text = message.get("content") if isinstance(message, dict) else None
        elif self.config.provider == "anthropic":
            data = self._post(f"{self.config.base_url}/v1/messages", {"model": self.config.model, "max_tokens": 1024, "system": system, "messages": [{"role": "user", "content": user}]}, {"x-api-key": self.config.api_key, "anthropic-version": "2023-06-01"})
            content = data.get("content")
            first = content[0] if isinstance(content, list) and content else None
            text = first.get("text") if isinstance(first, dict) else None
        else:
            data = self._post(f"{self.config.base_url}/models/{self.config.model}:generateContent?key={self.config.api_key}", {"systemInstruction": {"parts": [{"text": system}]}, "contents": [{"role": "user", "parts": [{"text": user}]}], "generationConfig": {"temperature": 0}}, {})
